...
class BareboneBuilder:
    ...
    def build_kernel(self):
        ...
        self.execute_command("c:\\nasm\\nasm.exe boot.asm -o k.bin",False)
        self.execute_command("as.exe --32 boot.S -o b.o",False)
        fff=f'gcc -m32 -o3 -lgcc -c "$1" -o kernel.o -nostdlib'.replace("$1",filename)
        
        self.execute_command(fff,True)
        self.execute_command("ldc.exe -T link.ld b.o kernel.o -o ke.exe -nostdlib",True)
        self.execute_command("objcopy.exe -O binary .\\ke.exe kernel.c32",True)
        
        self.execute_command('c:\\windows\\system32\\cmd.exe /c "copy /b k.bin+kernel.c32 kernel.bin "',True)
    def run_kernel(self):
        self.text_area.delete(1.0, tk.END)
        self.execute_command("qemu-system-x86_64.exe -boot a -fda kernel.bin",True)
